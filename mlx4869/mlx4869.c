...
#define RAM_ACCESS		0x00 //RAM aess mand RAM存取命令
#define EEPROM_ACCESS	0x20 //EEPROM aess mand EEPROM存取命令
#define RAM_TOBJ1		0x07 //To1 address in the eeprom 目标1温度,检测到的红外温度 -70.01 ~ 382.19度//
#define SMBUS_PORT	GPIOA      //PB端口(端口和下面的两个针脚可自定义)
...
#define SMBUS_SCK_H()	    HAL_GPIO_WritePin(GPIOA,GPIO_PIN_6,GPIO_PIN_SET)//SMBUS_PORT->BSRR = SMBUS_SCK //置高电平
#define SMBUS_SCK_L()	    HAL_GPIO_WritePin(GPIOA,GPIO_PIN_6,GPIO_PIN_RESET)//SMBUS_PORT->BRR = SMBUS_SCK  //置低电平
...
#define SMBUS_SDA_PIN()	    HAL_GPIO_ReadPin(GPIOA,GPIO_PIN_7)//SMBUS_PORT->IDR & SMBUS_SDA //读取引脚电平/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*//*******************************************************************************
...
void SMBus_StartBit(void)
{
	...
	SMBus_Delay(5);	    // Hold time after (Repeated) Start// Condition. After this period, the first clock is generated.//(Thd:sta=4.0us min)在SCK=1时，检测到SDA由1到0表示通信开始（下降沿）
...
void SMBus_StopBit(void)
{
	...
	SMBUS_SDA_H();
	SMBus_Delay(5);// Set SDA line在SCK=1时，检测到SDA由0到1表示通信结束（上升沿）
}/*******************************************************************************
